...
public sealed class PremiumPaymentServiceTests
{
    ...
    private sealed class FakeRepository : IPaymentRepository
    {
        ...
        public Task<PaymentOrder> CreateOrderAsync(long userId, PremiumPlan plan, DateTimeOffset expiresAt, CancellationToken cancellationToken)
        {
            ...
            {
                Id = 10, OrderCode = 1, UserId = userId, Plan = plan.Code, Amount = plan.Amount,
                Status = PaymentOrderStatus.Created, ExpiresAt = expiresAt, CreatedAt = Now, UpdatedAt = Now
            });
        }
        ...
        public Task<PaymentOrder?> GetOwnedOrderAsync(long userId, long orderCode, CancellationToken cancellationToken) => Task.FromResult<PaymentOrder?>(null);
